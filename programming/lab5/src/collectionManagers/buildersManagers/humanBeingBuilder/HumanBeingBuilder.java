...
public class HumanBeingBuilder implements ObjectBuilder<HumanBeing> {
    ...
    public HumanBeing build() throws IllegalArgumentException{
        try {
            ...
            while (true){
                try {
                    System.out.println("Введите значение hasToothpick (тип - boolean, не может быть пустым):");
                    nextLine = scanner.nextLine();
                    if (inputValidator.validate(nextLine)) {
                        if (!nextLine.equalsIgnoreCase("true") && !nextLine.equalsIgnoreCase("false")){
                            throw new InputMismatchException();
            ...
            while (true) {
                try {
                    System.out.println("Введите значение impactSpeed (тип - Long, значение может быть пустым):");
                    nextLine = scanner.nextLine();
                    if (nextLine.isBlank()){
                        impactSpeed = null;
                    }
            ...
            while (true) {
                try {
                    System.out.println("Введите значение minutesOfWaiting (тип - long, не может быть пустым):");
                    nextLine = scanner.nextLine();
                    if (inputValidator.validate(nextLine)){
